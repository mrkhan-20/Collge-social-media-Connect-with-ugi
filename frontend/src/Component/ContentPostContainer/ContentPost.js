...
export default function ContentPost() {
  ...
  const handlePost = (e)=>{
    e.preventDefault();
    fetch(`http://localhost:5000/api/post/user/sentiment` , {method:"POST" , headers:{'Content-Type':"application/JSON", token : accessToken} ,
     ...
     .then((data)=>{
        if(data.status==200){
          ...
          alert("Your post contains some inappropriate meaning that should not be on this platform");
        }
     })
